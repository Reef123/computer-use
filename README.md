...
The goal is the cheapest way to get certain. Like walking up a ladder. Bottom rung is
cheap. Top rung is expensive.

STEPS:

1. Look again at the screen it already has.
2. Probe one spot. Ask the accessibility tree what is under the cursor.
3. Resample the model. Ask it a few times and see if it agrees with itself.
4. Act.
...
A policy core. It turns what the agent does not know into one decision. Pure and
testable.
...
question. Is it cheaper without getting more wrong?
...
The A/B came back. The task was nine steps. The cheap policy checked zero times. The
full-look baseline checked all nine. The cheap one won on cost. Then it committed the one
consequential action blind. It failed on safety.

So I tested the assumption under everything. Resampling came back flat. 0.00 on almost
every step. 0.00 on the consequential one. The reason is structural. Resampling measures
...
The model is calm and sure right up to the irreversible thing. The calm is the danger
area. Neither its confidence nor the action type tells you the thing is irreversible.
...
The model can do one read. It cannot do the other. So it should be an automation router.
Easy steps get a small model. Hard steps get a big one. Safe steps act. Risky steps slow
down.
...
Two builds. First the second read. The probe already reads the words on a button. It can
read "Replace all, no undo" the same way. Same loop, same probe, one more thing to listen
for. Then the router around both reads, so the easy steps run cheap and the rare dangerous
ones get the slow, careful path.

## Run

    python tests/test_policy.py     # the policy, on deterministic input
    python ab_demo.py               # the A/B cost-vs-safety check, on a fixture
